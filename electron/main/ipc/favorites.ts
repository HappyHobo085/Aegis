 ...
 * updated Favorite[] so the renderer syncs from the result. (Tagging lives on the
 * saved list — see ipc/saved.ts.)
 */
export function buildFavoritesHandlers(
  ...
  return {
    ...
    [IPC.favoritesAdd]: (input: { name: string; url: string }): Favorite[] => repo.add(input),
    [IPC.favoritesUpdate]: (
      id: number,
