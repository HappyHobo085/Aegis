...
(function () {
  try {
        ...
        return sdp
          ...
          .filter(function (line) {
            if (line.indexOf('a=candidate:') === 0 || line.indexOf('candidate:') === 0)
              return keepCand(line);
            return true;
          })
          .map(function (line) {
            if (line.indexOf('c=IN IP4 ') === 0) {
              return isLocalAddr(line.slice(9).trim()) ? 'c=IN IP4 0.0.0.0' : line;
            }
            if (line.indexOf('c=IN IP6 ') === 0) {
              return isLocalAddr(line.slice(9).trim()) ? 'c=IN IP6 ::' : line;
            }
            if (line.indexOf('o=') === 0) {
              var parts = line.split(' ');
              if (parts.length === 6 && isLocalAddr(parts[5])) {
                parts[5] = parts[4] === 'IP6' ? '::' : '0.0.0.0';
                return parts.join(' ');
              }
            }
            return line;
      ...
      try {
        ...
        var realRemoveEL =
          typeof pc.removeEventListener === 'function'
            ? pc.removeEventListener.bind(pc)
            : function () {};
        function filteredIce(fn) {
          return function (ev) {
            try {
              if (ev && ev.candidate && ev.candidate.candidate && !keepCand(ev.candidate.candidate))
                return;
            } catch (e) {}
        ...
        pc.addEventListener = function (type, fn, opts) {
          if (type === 'icecandidate' && typeof fn === 'function') {
            ...
            try {
              Object.defineProperty(fn, '__aegisIceWrap', { value: w, configurable: true });
            } catch (e) {}
        ...
        try {
          Object.defineProperty(pc, 'onicecandidate', {
            configurable: true,
            get: function () {
              return this.__aegisOnIce || null;
            },
            set: function (fn) {
              if (this.__aegisOnIceWrap) {
                try {
                  realRemoveEL('icecandidate', this.__aegisOnIceWrap);
                } catch (e) {}
                this.__aegisOnIceWrap = null;
              }
              this.__aegisOnIce = typeof fn === 'function' ? fn : null;
              if (typeof fn === 'function') {
                var w = filteredIce(fn);
                this.__aegisOnIceWrap = w;
                realAddEL('icecandidate', w);
              }
            },
          });
        } catch (e) {}
          ...
          pc[m] = function () {
            ...
            if (r && typeof r.then === 'function') {
              return r.then(function (desc) {
                try {
                  if (desc && desc.sdp) return { type: desc.type, sdp: filterSdp(desc.sdp) };
                } catch (e) {}
                return desc;
              });
            }
        ...
        ['localDescription', 'currentLocalDescription', 'pendingLocalDescription'].forEach(
          function (p) {
            try {
              var d = Object.getOwnPropertyDescriptor(OrigPC.prototype, p);
              if (!d || !d.get) return;
              Object.defineProperty(pc, p, {
                configurable: true,
                get: function () {
                  var v = d.get.call(this);
                  try {
                    if (v && v.sdp) return { type: v.type, sdp: filterSdp(v.sdp) };
                  } catch (e) {}
                  return v;
                },
              });
                    ...
                    report.forEach(function (stat, id) {
                      ...
                      try {
                        if (
                          ...
                          if (s.relatedAddress && isLocalAddr(s.relatedAddress))
                            s.relatedAddress = null;
                        }
                      } catch (e) {
                        s = stat;
                      }
    ...
    function Patched() {
      return wrap(new OrigPC(...arguments));
    }
    ...
    try {
      Object.getOwnPropertyNames(OrigPC).forEach(function (k) {
        if (!(k in Patched)) {
          try {
            Object.defineProperty(Patched, k, Object.getOwnPropertyDescriptor(OrigPC, k));
          } catch (e) {}
        }
      });
    } catch (e) {}
